    ...
    public sealed partial class MainPage : Page
    {
        ...
        private void Create_Clicked(object sender, RoutedEventArgs e)
        {
            ...
            if (title.Text != "" && details.Text != "" && !judge)
            {
                ...
                using (var item = db.Prepare("INSERT INTO Items (title, details, datetime) VALUES(?, ?, ?)"))
                {
                    ...
                    item.Bind(3, date.Date.ToString("yyyy/MM/dd hh:mm:ss", DateTimeFormatInfo.InvariantInfo));
                    item.Step();
                }
        ...
        private void BtnGetAll_Click(object sender, RoutedEventArgs e)
        {
            ...
            using (var statement = App.conn.Prepare("SELECT id, title, details, datetime FROM Items WHERE id LIKE ? OR title LIKE ? OR details LIKE ? OR datetime LIKE ?"))
            {
                ...
                {
                    os += "Id: " + statement[0] + "; Title: " + statement[1] + "; Details: " + statement[2] + "; Date: " + statement[3] + "\n";
                }
                if (os != "")
                {
                ...
                {
                    var i = new MessageDialog("No such item has the tips named" + Query.Text).ShowAsync();
                }
            }
        }
